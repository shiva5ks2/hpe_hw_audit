...
def connect_enclosure (hostname,username,password,command, outdir):
	...
	try:
		...
		out_file = open (outdir + time.strftime("%Y-%m-%d") + '_ProCurve_' + hostname + '.txt','w')
...
def get_args():
	...
	parser = argparse.ArgumentParser(
				description='Script to get ProCurve information')
	# Add arguments
	parser.add_argument('-i', '--inputcsv', type=str, help='Input CSV that contains the ProCurve login credentials. Format : <IP address>,<UserID>,<Password>', required=True)
